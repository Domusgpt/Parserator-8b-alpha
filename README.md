...
- Monitor real-time plan health with `session.getPlanHealth()`—track confidence trends, failure rate, and refresh recommendations before agents or ops teams hit degraded data quality.【F:active-development/packages/core/src/session.ts†L300-L420】【F:active-development/packages/core/src/types.ts†L241-L274】
...
console.log(session.getPlanHealth());
```
