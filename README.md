...
- A Claude Code plugin lives under `claude/plugins/dev-marketplace`, bundling `/parserator-status` and `/parserator-parse` commands that now wrap executable helpers for fetching snapshots and running parses with metadata-rich output, plus a runbook for translating recommended actions into ops workflows.【F:claude/plugins/dev-marketplace/.claude-plugin/marketplace.json†L1-L11】【F:claude/plugins/dev-marketplace/parserator-lean-orchestration/scripts/status.mjs†L1-L45】【F:docs/LEAN_ORCHESTRATION_RUNBOOK.md†L1-L67】
