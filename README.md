...
- Register kernel-level interceptors with `core.use()` (or pass them at construction) to run custom hooks before parses start, short-circuit or rewrite requests/responses, and observe success/failure lifecycles across both direct calls and sessions without touching orchestration.【F:active-development/packages/core/src/index.ts†L200-L420】【F:active-development/packages/core/src/session.ts†L80-L520】
