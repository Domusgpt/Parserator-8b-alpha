...
- Persist SearchPlans automatically across parses with the built-in plan cache (swap it via `planCache`, tune TTL/LRU behaviour with `planCacheOptions`, or reuse the exported `createInMemoryPlanCache`) so repeated schemas skip architect costs, and inspect cache health at runtime through `core.getPlanCacheStats()`.【F:active-development/packages/core/src/index.ts†L74-L360】【F:active-development/packages/core/src/session.ts†L720-L870】【F:active-development/packages/core/src/cache.ts†L1-L170】【F:active-development/packages/core/src/types.ts†L240-L310】
