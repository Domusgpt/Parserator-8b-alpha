...
> 🧠 **Agent quick-start**: from the repo root run `npm run onboarding` to print the command cheat sheet, memorise the bootstrap + CLI flow, and then follow the steps below for your first parse.

### **First CLI Parse**
```bash
cd Parserator-8b-alpha/active-development
npm install
export PARSERATOR_API_KEY="<your-key>"  # grab from https://app.parserator.com
npm run build --workspace @parserator/sdk-node
npm run example:basic --workspace @parserator/sdk-node
```
> The basic example will sanity-check your key, run a quickParse walkthrough, and surface telemetry so you know the stack is healthy before touching code.
