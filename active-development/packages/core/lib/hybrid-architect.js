...
function createHybridArchitect(options) {
    const { base, client, logger, minHeuristicConfidence = DEFAULT_MIN_CONFIDENCE, concurrency = 1, cooldownMs = DEFAULT_COOLDOWN_MS, emitTelemetry } = options;
    ...
    return {
        ...
        getPlanRewriteState() {
            const pendingCooldown = cooldownWindow > 0 && lastAttemptAt > 0 && Date.now() - lastAttemptAt < cooldownWindow;
            return {
                enabled: true,
