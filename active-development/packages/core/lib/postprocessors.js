...
exports.executePostprocessors = executePostprocessors;
exports.createDefaultPostprocessors = createDefaultPostprocessors;
const utils_1 = require("./utils");
const WARNING_CONFIDENCE = 0.75;
function cloneMetadata(metadata) {
    const stageBreakdown = { ...metadata.stageBreakdown };
    ...
    return {
        ...metadata,
        architectPlan: (0, utils_1.clonePlan)(metadata.architectPlan, metadata.architectPlan.metadata.origin),
        diagnostics: [...metadata.diagnostics],
        stageBreakdown
    };
}
function mergeStageBreakdown(base, patch) {
    if (!patch) {
        return base;
    }
    const merged = { ...base };
...
function mergeMetadata(base, patch) {
    ...
    if (patch.architectPlan) {
        architectPlan = (0, utils_1.clonePlan)(patch.architectPlan, patch.architectPlan.metadata.origin);
    }
...
function createDiagnostic(message, severity = 'info') {
    return {
        field: '*',
        stage: 'postprocess',
        message,
...
async function executePostprocessors(postprocessors, context) {
    ...
    let metadata = cloneMetadata(context.metadata);
    const diagnostics = [];
    let totalTime = 0;
    let runs = 0;
    for (const postprocessor of postprocessors) {
        ...
        try {
            ...
            diagnostics.push(createDiagnostic(`${postprocessor.name} postprocessor failed: ${message}`, 'error'));
        }
        finally {
    ...
    postprocessors.push({
        ...
        run: ({ parsedData }) => {
            ...
            return {
                parsedData: pruned,
                diagnostics: [
                    createDiagnostic(`Removed ${removed.length} empty output fields for cleaner downstream payloads.`, 'info')
                ]
            };
        }
    });
