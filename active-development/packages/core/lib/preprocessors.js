...
exports.executePreprocessors = executePreprocessors;
exports.createDefaultPreprocessors = createDefaultPreprocessors;
const WARNING_CONFIDENCE = 0.75;
function normaliseRequestMerge(base, patch) {
    const merged = {
        ...base,
        ...patch
    };
...
async function executePreprocessors(preprocessors, context) {
...
function createDefaultPreprocessors(logger) {
    const preprocessors = [];
    preprocessors.push({
        name: 'trim-input',
        run: ({ request }) => {
    ...
    preprocessors.push({
        ...
        run: ({ request, config }) => {
            ...
            if (trimmedKeys.length > config.maxSchemaFields) {
                return {
                    diagnostics: [
                        createDiagnostic('Schema key normalization skipped due to schema exceeding max field threshold.', 'warning')
                    ]
                };
            }
            const normalizedSchema = trimmedKeys.reduce((acc, key, index) => {
                acc[key] = request.outputSchema[keys[index]];
                return acc;
            }, {});
            return {
                request: {
