...
describe('LeanLLMResolver', () => {
  ...
  it('allows optional fields when runtime overrides enable them', async () => {
    const client = new MockClient(async () => ({ value: 'runtime-optional', confidence: 0.66 }));
    const resolver = createResolver({ client });
    const context = createContext({
      step: {
        ...createContext().step,
        targetKey: 'optional_field',
        description: 'Optional field override',
        searchInstruction: 'Extract the optional field if available.',
        isRequired: false
      },
      options: {
        leanLLM: {
    ...
    expect(client.calls).toHaveLength(1);
    expect(result?.value).toBe('runtime-optional');

    const summary = context.shared.get(LEAN_LLM_USAGE_KEY) as LeanLLMFallbackUsageSummary;
    expect(summary.totalInvocations).toBe(1);
    expect(summary.fields[0]).toMatchObject({ field: 'optional_field', action: 'invoked' });
  });
  ...
  it('respects runtime token budgets when provided', async () => {
    const client = new MockClient(async () => ({ value: 'runtime-pass', confidence: 0.71, tokensUsed: 120 }));
    const resolver = createResolver({ client });
    const shared = new Map([[PLAN_SHARED_STATE_KEY, plan]]);
    const runtimeOptions = {
      leanLLM: {
        maxTokensPerParse: 100
      }
    };

    const firstContext = createContext({ shared, options: runtimeOptions });
    const secondContext = createContext({
      shared,
      options: runtimeOptions,
      step: {
        ...createContext().step,
        targetKey: 'secondary_field',
        description: 'Secondary field',
        searchInstruction: 'Extract the secondary field value.',
    ...
    expect(first?.value).toBe('runtime-pass');
    expect(second).toBeUndefined();
    expect(client.calls).toHaveLength(1);

    const summary = shared.get(LEAN_LLM_USAGE_KEY) as LeanLLMFallbackUsageSummary;
    expect(summary.maxTokensPerParse).toBe(100);
    expect(summary.skippedByLimits).toBe(1);
    expect(summary.fields.some(field => field.reason === 'token-budget')).toBe(true);
  });
  ...
  it('uses runtime default confidence when the model omits it', async () => {
    const client = new MockClient(async () => ({ value: '$777.00' }));
    const resolver = createResolver({ client, defaultConfidence: 0.42 });
    const context = createContext({
      options: {
        leanLLM: {
