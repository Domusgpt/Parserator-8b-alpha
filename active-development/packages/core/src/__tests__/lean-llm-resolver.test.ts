...
describe('LeanLLMResolver', () => {
  ...
  const createContext = (overrides: Partial<FieldResolutionContext> = {}): FieldResolutionContext => ({
    ...
    config: baseConfig,
    logger,
    shared: new Map<string, unknown>([[PLAN_SHARED_STATE_KEY, plan]]),
    ...overrides
  });
  ...
  it('honours runtime invocation limits provided via options', async () => {
    const client = new MockClient(async () => ({ value: 'first-pass', confidence: 0.7 }));
    const resolver = createResolver({ client });
    const shared = new Map<string, unknown>([[PLAN_SHARED_STATE_KEY, plan]]);
    ...
    const firstContext = createContext({
      shared,
      options: { leanLLM: { maxInvocationsPerParse: 1 } }
    });
    const secondContext = createContext({
      shared,
      step: {
    ...
    const summary = shared.get(LEAN_LLM_USAGE_KEY) as LeanLLMFallbackUsageSummary;
    expect(summary.totalInvocations).toBe(1);
    expect(summary.maxInvocationsPerParse).toBe(1);
    expect(summary.fields[1]).toMatchObject({ limit: 1, limitType: 'invocations' });
  });
  ...
  it('allows optional fields when runtime overrides permit it', async () => {
    const client = new MockClient(async () => ({ value: 'Processed optional note', confidence: 0.66 }));
    const resolver = createResolver({ client });
    ...
    const context = createContext({
      step: {
        ...
        searchInstruction: 'Locate any note',
        validationType: 'string',
        isRequired: false
      },
      options: { leanLLM: { allowOptionalFields: true } }
    });

    const result = await resolver.resolve(context);

    expect(result?.value).toBe('Processed optional note');
    expect(client.calls).toHaveLength(1);

    const summary = context.shared.get(LEAN_LLM_USAGE_KEY) as LeanLLMFallbackUsageSummary;
    expect(summary.totalInvocations).toBe(1);
    expect(summary.fields[0]).toMatchObject({ field: 'notes', action: 'invoked' });
  });

  it('skips invocation when runtime override disables the fallback', async () => {
    const client = new MockClient(async () => ({ value: '$321.00' }));
    const resolver = createResolver({ client });
    const context = createContext({ options: { leanLLM: { disabled: true } } });

    const result = await resolver.resolve(context);

    expect(result).toBeUndefined();
    expect(client.calls).toHaveLength(0);
    expect(context.shared.get(LEAN_LLM_USAGE_KEY)).toBeUndefined();
  });

  it('applies runtime plan confidence gate overrides', async () => {
    const client = new MockClient(async () => ({ value: '$999.00' }));
    const resolver = createResolver({ client, planConfidenceGate: 0.95 });
    const gatedPlan: SearchPlan = {
      ...plan,
      metadata: { ...plan.metadata, plannerConfidence: 0.68 }
    };

    const context = createContext({
      shared: new Map<string, unknown>([[PLAN_SHARED_STATE_KEY, gatedPlan]]),
      options: { leanLLM: { planConfidenceGate: 0.65 } }
    });
    ...
    expect(summary.totalInvocations).toBe(0);
    expect(summary.skippedByPlanConfidence).toBe(1);
    expect(summary.planConfidenceGate).toBe(0.65);
    expect(summary.fields[0]).toMatchObject({
      action: 'skipped',
      gate: 0.65,
      plannerConfidence: 0.68
    });
  });
