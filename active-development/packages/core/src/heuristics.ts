...
interface InstructionHintsResult {
  generalInstructions?: string;
  fieldHints: Map<string, string>;
}

function deriveInstructionHints(instructions?: string): InstructionHintsResult {
  if (!instructions) {
  ...
  const pushHint = (key: string, hint: string) => {
...
export function buildPlannerSteps(
  ...
  return Object.keys(outputSchema).map(field => {
    ...
    const fieldInstruction =
      instructionHints.fieldHints.get(fieldKey) ?? instructionHints.fieldHints.get(humanKeyNormalised);
