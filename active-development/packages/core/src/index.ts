...
export class ParseratorCore {
  ...
  describePipeline(): ParseratorPipelineDescriptor {
    const config = this.getConfig();
    const preprocessors = this.getPreprocessors().map(preprocessor => ({
      name: preprocessor.name
    }));
    const resolvers = this.listResolvers().map(name => ({ name }));
    const interceptors = this.describeInterceptors(this.getInterceptors());
    const telemetryListeners = this.telemetry.listeners()?.length ?? 0;

    return {
      profile: this.profileName,
      config,
      architect: this.describeAgent(this.architect),
      extractor: this.describeAgent(this.extractor),
      preprocessors,
      resolvers,
      interceptors,
      telemetry: {
