...
import {
  ...
  ParseratorStreamEvent,
  ParseratorStreamListener,
...
interface ParseRuntimeOptions {
  requestId?: string;
  streamListener?: ParseratorStreamListener;
}
...
export class ParseratorCore {
  ...
  async parse(request: ParseRequest): Promise<ParseResponse> {
    return this.executeParse(request);
  }

  async *parseStream(
    request: ParseRequest
  ): AsyncGenerator<ParseratorStreamEvent> {
    const events: ParseratorStreamEvent[] = [];
    ...
    const listener: ParseratorStreamListener = event => {
      events.push(event);
      flush();
    };

    const pipelinePromise = this.executeParse(request, { streamListener: listener });
  ...
  private async executeParse(
    ...
    const requestId = runtime.requestId ?? uuidv4();
    const streamListener = runtime.streamListener;
    ...
    const startEvent: ParseratorTelemetryEvent = {
      type: 'parse:start',
      source: 'core',
      requestId,
    ...
    await this.notifyStream(startEvent, streamListener);
    this.telemetry.emit(startEvent);
    ...
    try {
      ...
      const failureEvent: ParseratorTelemetryEvent = {
        type: 'parse:failure',
        source: 'core',
        requestId,
      ...
      await this.notifyStream(failureEvent, streamListener);
      this.telemetry.emit(failureEvent);
      ...
      await this.notifyStream(this.createResponseEvent(response, requestId), streamListener);
      return response;
    }
    ...
    const architectStageEvent: ParseratorTelemetryEvent = {
      type: 'parse:stage',
      source: 'core',
      requestId,
      timestamp: new Date().toISOString(),
    ...
    await this.notifyStream(architectStageEvent, streamListener);
    this.telemetry.emit(architectStageEvent);
    ...
    const planReadyEvent: ParseratorTelemetryEvent = {
      type: 'plan:ready',
      source: 'core',
      requestId,
      timestamp: new Date().toISOString(),
    ...
    await this.notifyStream(planReadyEvent, streamListener);
    this.telemetry.emit(planReadyEvent);
    ...
    const extractorStageEvent: ParseratorTelemetryEvent = {
      type: 'parse:stage',
      source: 'core',
      requestId,
      timestamp: new Date().toISOString(),
    ...
    await this.notifyStream(extractorStageEvent, streamListener);
    this.telemetry.emit(extractorStageEvent);
    ...
    if (error) {
      ...
      const successEvent: ParseratorTelemetryEvent = {
        type: 'parse:success',
        source: 'core',
        requestId,
        timestamp: metadata.timestamp,
      ...
      await this.notifyStream(successEvent, streamListener);
      this.telemetry.emit(successEvent);
  ...
  private createResponseEvent(response: ParseResponse, requestId: string): ParseratorStreamEvent {
    return {
      type: 'parse:response',
      source: 'core',
      requestId,
  ...
  private async notifyStream(
    event: ParseratorStreamEvent,
    listener?: ParseratorStreamListener
  ): Promise<void> {
    if (!listener) {
      return;
    }
    ...
    try {
      ...
      this.logger.warn?.('parserator-core:stream-listener-error', {
        error: error instanceof Error ? error.message : error,
  ...
  private async handleArchitectFailure(params: {
    ...
    const { request, architectResult, requestId, startTime, streamListener } = params;
  ...
  private async handleExtractorFailure(params: {
    ...
    const { requestId, architectResult, extractorResult, startTime, request, streamListener } = params;
