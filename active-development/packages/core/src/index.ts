...
export { createInMemoryPlanCache } from './cache';
...
export class ParseratorCore {
  ...
  constructor(options: ParseratorCoreOptions) {
    ...
    this.planCache =
      options.planCache === null
        ? undefined
        : options.planCache ?? createInMemoryPlanCache(options.planCacheOptions);
  ...
  async getPlanCacheStats(): Promise<ParseratorPlanCacheStats | undefined> {
    if (!this.planCache || typeof this.planCache.stats !== 'function') {
      return undefined;
    }

    try {
      const stats = await Promise.resolve(this.planCache.stats());
      return stats;
    } catch (error) {
      this.logger.warn?.('parserator-core:plan-cache-stats-failed', {
