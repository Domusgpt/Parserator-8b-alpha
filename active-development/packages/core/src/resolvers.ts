...
export const LEAN_LLM_RUNTIME_CONFIG_KEY = 'resolver:leanllm:runtime-config';

type LeanLLMResolverOptions = Omit<LeanLLMResolverConfig, 'position'> & {
  logger?: CoreLogger;
};

type EffectiveLeanLLMConfig = {
  disabled: boolean;
...
export class LeanLLMResolver implements FieldResolver {
  ...
  private readonly baseAllowOptionalFields: boolean;
  private readonly baseDefaultConfidence: number;
  private readonly baseMaxInputCharacters?: number;
  private readonly clientName: string;
  private readonly basePlanConfidenceGate?: number;
  private readonly baseMaxInvocationsPerParse?: number;
  private readonly baseMaxTokensPerParse?: number;
  ...
  private resolveEffectiveConfig(context: FieldResolutionContext): EffectiveLeanLLMConfig {
    ...
    return {
      ...
      maxInvocationsPerParse:
        runtime?.maxInvocationsPerParse ?? this.baseMaxInvocationsPerParse,
  ...
  private mergeRuntimeOptions(
    context: FieldResolutionContext
  ): LeanLLMRuntimeOptions | undefined {
    const sharedOptions = this.extractRuntimeOptions(
      context.shared.get(LEAN_LLM_RUNTIME_CONFIG_KEY)
    );
  ...
  private extractRuntimeOptions(raw: unknown): LeanLLMRuntimeOptions | undefined {
    if (!raw || typeof raw !== 'object' || Array.isArray(raw)) {
      return undefined;
    }

    return raw as LeanLLMRuntimeOptions;
  }
  ...
  private toResolution(
    ...
    config: EffectiveLeanLLMConfig
  ): FieldResolutionResult {
    const baseConfidence = clamp(response.confidence ?? config.defaultConfidence, 0, 1);
    const confidence =
  ...
  private tryReuseSharedExtraction(
    ...
    const confidence = clamp(existing.confidence ?? config.defaultConfidence, 0, 1);
    const diagnostics: ParseDiagnostic[] = [
      {
        field: context.step.targetKey,
        stage: 'extractor',
  ...
  private trimInput(input: string, config: EffectiveLeanLLMConfig): string {
