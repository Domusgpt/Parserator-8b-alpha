...
import {
  ...
  RefreshPlanOptions,
  ParseratorPlanHealth,
  ParseratorPlanRecommendation,
  ParseratorConfidenceTrend
} from './types';
import {
  clamp,
  createEmptyPlan,
...
export class ParseratorSession {
  ...
  private planRuns = 0;
  private planSuccesses = 0;
  private planFailures = 0;
  private planConfidenceHistory: number[] = [];
  private readonly maxConfidenceSamples = 50;
  ...
  getPlanHealth(): ParseratorPlanHealth {
    ...
    const notes: string[] = [];
    let recommendation: ParseratorPlanRecommendation = 'monitor';
  ...
  private resetPlanHealthMetrics(): void {
    this.planRuns = 0;
  ...
  private recordPlanRun(params: { confidence?: number; success: boolean; timestamp: string }): void {
    if (!this.plan) {
      return;
    }
    ...
    if (typeof params.confidence === 'number' && !Number.isNaN(params.confidence)) {
      const sample = clamp(params.confidence, 0, 1);
  ...
  private calculateConfidenceTrend(): ParseratorConfidenceTrend {
