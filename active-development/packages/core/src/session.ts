...
import { evaluatePlanCacheEntry } from './cache';
import {
  clamp,
  createEmptyPlan,
...
interface ParseratorSessionDependencies {
  ...
  planCacheKey?: string;
  planCachePolicy?: ParseratorPlanCachePolicy;
  initialPlanCacheInfo?: ParseratorPlanCacheInfo;
...
export class ParseratorSession {
  ...
  private planCacheInfo?: ParseratorPlanCacheInfo;
  ...
  constructor(private readonly deps: ParseratorSessionDependencies) {
    ...
    this.planCachePolicy = deps.planCachePolicy ? { ...deps.planCachePolicy } : undefined;

    const initialPlanCacheInfo = deps.initialPlanCacheInfo
      ? createPlanCacheInfo(deps.initialPlanCacheInfo)
      : deps.init.planCacheInfo
        ? createPlanCacheInfo(deps.init.planCacheInfo)
        : createPlanCacheInfo({
    ...
    this.setPlanCacheInfo(initialPlanCacheInfo);
    this.deps.init.planCacheInfo = this.getPlanCacheInfoClone();
  ...
  private setPlanCacheInfo(info?: ParseratorPlanCacheInfo): void {
    this.planCacheInfo = info ? clonePlanCacheInfo(info) : undefined;
  }

  private getPlanCacheInfoClone(): ParseratorPlanCacheInfo | undefined {
    return clonePlanCacheInfo(this.planCacheInfo);
  }
  ...
  async parse(inputData: string, overrides: SessionParseOverrides = {}): Promise<ParseResponse> {
    ...
    const response: ParseResponse = {
      ...
      metadata: {
        ...
        planCache: this.getPlanCacheInfoClone()
      },
      error
    };
  ...
  exportInit(overrides: Partial<ParseratorSessionInit> = {}): ParseratorSessionInit {
    ...
    return {
      ...
      planCacheInfo: overrides.planCacheInfo ?? this.getPlanCacheInfoClone()
    };
  }
  ...
  private queuePlanCachePersist(reason: string): void {
    ...
    this.setPlanCacheInfo(
      createPlanCacheInfo({
        ...this.planCacheInfo,
        key: this.planCacheKey,
        status: this.planCacheInfo?.status ?? (this.planCache ? 'miss' : 'disabled'),
        stale: false,
        ageMs: 0,
  ...
  private async ensurePlan(params: {
    ...
    const planCachePolicy = this.planCachePolicy;
    ...
    this.setPlanCacheInfo(planCacheInfo);
    ...
    if (params.reason === 'ensure' && this.planCache && this.planCacheKey) {
      try {
        ...
        planCacheInfo = createPlanCacheInfo({
          ...
          updatedAt: cacheResolution.updatedAt ?? planCacheInfo.updatedAt,
          profile: cached?.profile ?? this.profileName,
          reason: cacheResolution.reason
        });
        this.setPlanCacheInfo(planCacheInfo);

        if (cacheResolution.entry?.plan && (cacheResolution.status === 'hit' || cacheResolution.status === 'stale')) {
    ...
    planCacheInfo = createPlanCacheInfo({
      ...
      profile: this.profileName,
      refreshed: true,
      refreshedAt: this.planUpdatedAt,
      policy: planCachePolicy
    });
    this.setPlanCacheInfo(planCacheInfo);
  ...
  async refreshPlan(options: RefreshPlanOptions = {}): Promise<ParseratorPlanRefreshResult> {
    ...
    const previousPlanCacheInfo = this.getPlanCacheInfoClone();
    ...
    if (failure) {
      ...
      this.setPlanCacheInfo(previousPlanCacheInfo);
      this.deps.init.planCacheInfo = this.getPlanCacheInfoClone();
      return {
        success: false,
        failure,
