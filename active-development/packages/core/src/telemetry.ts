...
import {
  ...
  ParseratorFieldFallbackSkipReason,
...
export function createFieldFallbackTelemetryEmitter(
  options: FieldFallbackTelemetryEmitterOptions
): FieldFallbackTelemetryEmitter {
  const requestIdFactory = options.requestIdFactory ?? uuidv4;
  ...
  const normaliseQueue = (
    queue: ParseratorLeanLLMFieldFallbackQueueState | undefined
  ): ParseratorLeanLLMFieldFallbackQueueState | undefined => {
    if (!queue) {
      return undefined;
    }
