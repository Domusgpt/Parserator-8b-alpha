...
export type ParseratorPlanCacheStatus =
  | 'disabled'
  | 'miss'
  | 'hit'
  | 'stale'
...
export interface ParseratorPlanCachePolicy {
  minConfidence?: number;
  staleAfterMs?: number;
  maxAgeMs?: number;
}

export interface ParseratorPlanCacheInfo {
  key?: string;
  status: ParseratorPlanCacheStatus;
  stale?: boolean;
  ageMs?: number;
  updatedAt?: string;
  profile?: string;
...
export interface ParseratorPlanCacheResolution {
  status: ParseratorPlanCacheStatus;
