...
export interface ParseDiagnostic {
  field: string;
  stage: 'preprocess' | 'validation' | 'architect' | 'extractor' | 'orchestration';
  message: string;
...
export interface ParseError {
  code: string;
  message: string;
  stage: 'validation' | 'preprocess' | 'architect' | 'extractor' | 'orchestration';
...
export type ParseratorInterceptorHookName = 'beforeParse' | 'afterParse' | 'onFailure';

export interface ParseratorPipelineComponentDescriptor {
  name: string;
  notes?: string[];
}

export interface ParseratorPipelineInterceptorDescriptor {
  name: string;
  hooks: ParseratorInterceptorHookName[];
}

export interface ParseratorPipelineDescriptor {
  profile?: string;
  config: ParseratorCoreConfig;
  architect: ParseratorPipelineComponentDescriptor;
  extractor: ParseratorPipelineComponentDescriptor;
  preprocessors: ParseratorPipelineComponentDescriptor[];
  resolvers: ParseratorPipelineComponentDescriptor[];
  interceptors: ParseratorPipelineInterceptorDescriptor[];
  telemetry: {
    listeners: number;
  };
  planCache: {
...
export interface ParseratorSessionDescriptor {
  sessionId: string;
  createdAt: string;
  profile?: string;
  pipeline: ParseratorPipelineDescriptor;
...
export interface ParseratorParseStageEvent extends ParseratorTelemetryBaseEvent {
  type: 'parse:stage';
  stage: 'preprocess' | 'architect' | 'extractor';
