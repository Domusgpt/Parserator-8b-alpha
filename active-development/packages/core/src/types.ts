...
export interface ParseratorPlanCacheEntry {
  ...
  profile?: string;
  /**
   * Optional expiry timestamp for caches that support TTL-based invalidation.
   * In-memory cache implementations set this automatically when a TTL policy
   * is active, but custom caches can elect to ignore it.
   */
  expiresAt?: string;
}

export interface ParseratorPlanCacheStats {
  size: number;
  hits: number;
  misses: number;
  sets: number;
...
export interface ParseratorInMemoryPlanCacheOptions {
  /**
   * Maximum number of plans to keep in memory. When exceeded the cache will
   * evict the least recently used entries. Defaults to unlimited.
   */
  maxEntries?: number;
  /**
   * Time-to-live in milliseconds for cached plans. Expired entries are
   * discarded lazily on access. Defaults to no TTL.
   */
  ttlMs?: number;
}
...
export interface ParseratorPlanCache {
  ...
  stats?(): Promise<ParseratorPlanCacheStats> | ParseratorPlanCacheStats;
}
...
export interface ParseratorCoreOptions {
  ...
  planCacheOptions?: ParseratorInMemoryPlanCacheOptions;
}
