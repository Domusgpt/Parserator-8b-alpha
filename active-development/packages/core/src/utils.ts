...
export interface FailureResponseOptions {
  ...
  planCache?: ParseratorPlanCacheInfo;
}
...
export function createFailureResponse(options: FailureResponseOptions): ParseResponse {
  ...
  const metadata: ParseMetadata = {
    ...
    planCache: options.planCache ? clonePlanCacheInfo(options.planCache) : undefined
  };
...
export function clonePlanCacheInfo(
  info?: ParseratorPlanCacheInfo
): ParseratorPlanCacheInfo | undefined {
  if (!info) {
    return undefined;
  }
  ...
  return {
    ...info,
    policy: info.policy ? { ...info.policy } : undefined
  };
}

export function createPlanCacheInfo(
  info: ParseratorPlanCacheInfo
): ParseratorPlanCacheInfo {
  return clonePlanCacheInfo(info) ?? {
    status: 'disabled'
  };
}
