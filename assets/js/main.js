...
const scrollProgressBars = document.querySelectorAll('[data-scroll-progress]');

if (scrollProgressBars.length) {
...
const navGroups = Array.from(document.querySelectorAll('[data-nav-links]'))
  .map((group) => {
...
if (navGroups.length) {
  const activateCurrent = () => {
    const currentOffset = window.scrollY + window.innerHeight * 0.2;

    navGroups.forEach(({ entries }) => {
      let activeEntry = entries[0];

      entries.forEach((entry) => {
        if (entry.target.offsetTop <= currentOffset) {
          activeEntry = entry;
        }
      });
