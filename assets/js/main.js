...
document.querySelectorAll('[data-playbook]').forEach((playbook) => {
  const tabs = playbook.querySelectorAll('[data-playbook-tab]');
  const panes = playbook.querySelectorAll('[data-playbook-pane]');
  ...
  const setActive = (key) => {
    ...
    tabs.forEach((tab) => {
      const isActive = tab.dataset.playbookTab === key;
    ...
    panes.forEach((pane) => {
      const isActive = pane.dataset.playbookPane === key;
      pane.classList.toggle('is-active', isActive);
      pane.setAttribute('aria-hidden', isActive ? 'false' : 'true');
    });
  };

  const defaultKey = playbook.dataset.playbookDefault || (tabs[0] && tabs[0].dataset.playbookTab);
  setActive(defaultKey);

  const focusTabAtIndex = (index) => {
    if (index < 0 || index >= tabs.length) return;
    const tab = tabs[index];
    setActive(tab.dataset.playbookTab);
    tab.focus();
  };

  tabs.forEach((tab, index) => {
    tab.addEventListener('click', () => setActive(tab.dataset.playbookTab));
