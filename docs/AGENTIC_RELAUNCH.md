...
For a tactical landing pad that pairs with this strategy brief, start with `AGENTS.md` (or blast `npm run onboarding` from repo root for the same command map). It gives new agents a WMA-aligned command map, directory cues, and update protocol that mirrors the architecture below—then run `cd active-development && npm install && npm run demo` to prove Parserator is parsing locally before deeper work.
