...
For a tactical landing pad that pairs with this strategy brief, start with `AGENTS.md` (or blast `npm run onboarding` from repo root for the same command map). The cheat sheet now walks through installing the workspace, building `@parserator/core`/`@parserator/sdk-node`, setting `PARSERATOR_API_KEY`, and running the CLI demos so every agent feels the parse loop before touching code.
