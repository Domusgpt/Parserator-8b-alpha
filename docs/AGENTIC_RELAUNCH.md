...
- **Streaming progress hooks** – subscribe to `ParseratorCore#parseStream` for real-time start/stage/plan/final events so builders can light up dashboards, chatty agents, or CLI progress bars without bolting on their own telemetry queue.【F:active-development/packages/core/src/index.ts†L216-L363】【F:active-development/packages/core/src/types.ts†L300-L344】
