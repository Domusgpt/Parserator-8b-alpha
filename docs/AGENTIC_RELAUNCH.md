...
- **Health analytics** – `session.getPlanHealth()` tracks trendlines, failure rate, and refresh recommendations so ops teams can catch drift long before parsed data degrades, keeping EMA promises intact.【F:active-development/packages/core/src/session.ts†L300-L420】【F:active-development/packages/core/src/types.ts†L241-L274】
