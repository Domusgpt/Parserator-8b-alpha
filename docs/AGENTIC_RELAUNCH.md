...
- **Plan cache and portability** – configure a shared `planCache` (or use the bundled `createInMemoryPlanCache`) so SearchPlans survive across workers and hydrated sessions, and now apply `planCachePolicy` thresholds (min confidence, stale/expiry windows) while reading per-parse `planCache` metadata to see hits, staleness, and refresh history in telemetry.【F:active-development/packages/core/src/index.ts†L300-L600】【F:active-development/packages/core/src/session.ts†L720-L1120】【F:active-development/packages/core/src/cache.ts†L1-L120】
