...
## Lean LLM Resolver Orchestration
The lean LLM fallback resolver joins the async ecosystem rather than bypassing it. Each model request is enqueued through `createAsyncTaskQueue`, ensuring concurrency stays capped, errors surface through shared diagnostics, and downstream listeners can keep counting pending vs. in-flight work. Because resolver results are cached in the shared parse context, the LLM is consulted only once per parse—even when multiple fields were unresolved—preserving the queue’s efficiency while honouring the deterministic pipeline defaults.
