...
if (!reduceMotion) {
  ...
  tiltElements.forEach((element) => {
    ...
    const handleMove = (event) => {
      ...
      element.style.setProperty('transform', `perspective(1200px) rotateX(${rotateX.toFixed(2)}deg) rotateY(${rotateY.toFixed(2)}deg)`);
      element.classList.add('is-tilting');
    };

    const resetTilt = () => {
      element.style.setProperty('transform', 'perspective(1200px) rotateX(0deg) rotateY(0deg)');
